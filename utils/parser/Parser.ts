...
class ResultTrackerImpl<T> implements ResultTracker<T> {
  ...
  update(result: ParseResult<T>): void {
    if (result.type === "Failure") {
      ...
      if (
        ...
        result.end > this.selectedResult.end
      ) {
  ...
  get(): ParseResult<T> {
    ...
    throw new Error(`UnexpectedError: Tracker did not receive result`);
  }
  getFailure() {
    if (this.selectedError) {
      return this.selectedError.error;
...
export function expectEOF<T>(result: ParseResult<T>): ParseResult<T> {
  if (result.type === "Success") {
    if (result.rest.empty === false) {
      throw new Error(`NotEOF: ${result.rest}`);
    }
  }
  return result;
}
...
export function stackToString(stack: Stack, indent = 0): string {
  const indentText = " ".repeat(indent);
  return stack
    .map((item) => `${indentText}${item.name}:${item.position} ${item.message}`)
    .join("\n");
}
