...
export function regexp<Ctx>(reg: RegExp, name?: string): RegexpParser<Ctx> {
  ...
  if (reg.source[0] !== "^") {
    throw new Error(
      `UnexpectedError: Regular expression should start with '^': ${nameResolved}`
    );
  }
  if (!reg.global) {
    throw new Error(
      `UnexpectedError: Regular expression should be global: ${nameResolved}`
    );
  }
  return {
    regexp: reg,
...
export function rule<T, Ctx>(name: string): Rule<T, Ctx> {
  ...
  return {
    setParser(p) {
      if (parser !== null) {
        throw new Error(`UnexpectedError: Parser already set !`);
      }
      parser = p;
    },
    parse(_parentPath, input, skip, ctx) {
      if (parser === null) {
        throw new Error(`ParserNotImplemented: ${name}`);
      }
      return parser.parse(
